...
namespace Task_C
{
    internal class Graph
    {
        ...
        public void RemoveNode(string name)
        {
            ...
            foreach (GraphNode n in nodes)
            {
                n.GetAdjList().Remove(removeNode);
            }
            nodes.Remove(removeNode);
        }

        public List<string> GetDirectAndIndirectFriends(string name)
        {
            GraphNode startnode = GetNodeByName(name);
            ...
            while (queue.Count > 0)
            {
                ...
                foreach (GraphNode neighbor in current.GetAdjList())
                {
            ...
            List<string> friendNames = visited.Select(n => n.Name).ToList();
